...
x = 10
data = Array.new(x) {Array.new(datasize)}
t2 = Time.now
puts "Acquire #{x} frames"
(0..x-1).each do |i|
  # Acquire
  ...
  data[i] = dataptr.read_array_of_int datasize
  #puts "Data acquired #{Time.now.to_f}"
end
puts "Data acquisition took #{Time.now - t2}"
t3 = Time.now
AndorLib.ShutDown
puts "Shutdown took #{Time.now - t3}"


fout = File.open("data#{Time.now.strftime("%d%b%Y-%H%M%S")}.dat", 'w')
